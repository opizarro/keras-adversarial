...
def model_generator(latent_dim, units=512, dropout=0.5, reg=lambda: l1l2(l1=1e-6, l2=1e-6)):
...
def model_encoder(latent_dim, input_shape, units=512, reg=lambda: l1l2(l1=1e-6, l2=1e-6), dropout=0.5):
    k = 5
    x = Input(input_shape)
    ...
    z = Lambda(lambda ml: ml[0] + K.random_normal(K.shape(ml[0])) * K.exp(ml[1] / 2),output_shape=lambda ml: ml[0])([mu, log_sigma_sq])

    return Model(x, z, name="encoder")


def model_discriminator(latent_dim, output_dim=1, units=256, reg=lambda: l1l2(1e-6, 1e-6)):
    z = Input((latent_dim,))
    h = z
    mode = 1
...
def main():
    example_aae("output/aae-benthic", AdversarialOptimizerSimultaneous())
